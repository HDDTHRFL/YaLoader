...
CLEAR_QUEUE_CONFIRMATION_DETAILS = "Все задачи из списка будут удалены. Уже скачанные файлы на диске не удаляются."
...
REPLACE_COOKIES_CONFIRMATION_DETAILS = (
    "Используйте только файл, который вы получили из своего браузера или yt-dlp. Не импортируйте чужие cookies."
)
...
YANDEX_COOKIES_INFO_TEXT = "Перед созданием cookies.txt откройте Яндекс Браузер и войдите в YouTube."
...
CLEAR_HISTORY_CONFIRMATION_DETAILS = "Записи истории будут удалены из YaLoader. Скачанные файлы на диске не удаляются."
...
class MainWindow(QMainWindow):
    ...
    def _handle_speed_limit_indicator_window_state_changed(self) -> None:
        ...
        self._speed_limit_indicator_uses_window_resize = self._should_resize_window_for_speed_limit_indicator()
    ...
    def _connect_signals(self) -> None:
        ...
        self._settings_panel.choose_downloads_dir_button.clicked.connect(self._handle_choose_downloads_dir_clicked)
        self._settings_panel.set_downloads_dir_clicked_callback(self._handle_open_downloads_dir_clicked)
        ...
        self._speed_settings_dialog.confirm_clear_queue_changed.connect(self._handle_confirm_clear_queue_changed)
        ...
        self._environment_panel.prepare_system_button.clicked.connect(self._handle_prepare_system_clicked)
        self._environment_panel.update_tools_button.clicked.connect(self._handle_update_tools_clicked)
        self._environment_panel.connect_ytdlp_reset_requested(self._handle_reset_ytdlp_clicked)
        self._environment_panel.import_cookies_action.triggered.connect(self._handle_import_cookies_clicked)
        self._environment_panel.export_firefox_cookies_action.triggered.connect(
            self._handle_export_firefox_cookies_clicked
        )
        self._environment_panel.export_opera_cookies_action.triggered.connect(self._handle_export_opera_cookies_clicked)
        ...
        self._environment_panel.delete_cookies_button.clicked.connect(self._handle_delete_cookies_clicked)
        self._environment_panel.refresh_button.clicked.connect(self._handle_refresh_environment_status_clicked)
        self._environment_panel.open_cookies_dir_button.clicked.connect(self._handle_open_cookies_dir_clicked)
        self._environment_panel.open_downloads_dir_button.clicked.connect(self._handle_open_downloads_dir_clicked)
    ...
    def _handle_history_panel_width_animation_value_changed(self, value: object) -> None:
        ...
        panel_width = round(
            ...
            + (self._history_animation_end_panel_width - self._history_animation_start_panel_width) * progress
        )
        window_width = round(
            self._history_animation_start_window_width
            + (self._history_animation_end_window_width - self._history_animation_start_window_width) * progress
        )
    ...
    def _handle_add_history_record_to_queue(self, record: DownloadHistoryRecord) -> None:
        self._apply_history_update(update=self._history_controller.add_record_to_queue(record=record))
    ...
    def _apply_metadata_result(self, *, result: MediaMetadataResolutionResult) -> None:
        ...
        if result.metadata is None:
            ...
            self._show_transient_status_message("Не удалось определить данные медиа. yt-dlp попробует скачать напрямую")
            return
        ...
        if result.metadata.requested_video_quality is VideoQuality.BEST:
            self._show_transient_status_message(f"Качество определено: {result.metadata.resolved_video_quality.value}")
            return
        ...
        if result.metadata.resolved_video_quality is not result.metadata.requested_video_quality:
            self._show_transient_status_message(
                f"Выбранное качество недоступно. Будет использовано: {result.metadata.resolved_video_quality.value}"
            )
            return

        self._show_transient_status_message(f"Качество подтверждено: {result.metadata.resolved_video_quality.value}")
    ...
    def _handle_add_to_queue_clicked(self) -> None:
        self._apply_queue_input_update(
            update=self._queue_input_controller.add_from_input(
                ...
                download_speed_limit_bytes_per_second=(self._settings.download_speed_limit_bytes_per_second),
            )
        )
    ...
    def _apply_download_speed_limit_change(self, *, bytes_per_second: int | None) -> None:
        ...
        self._show_transient_status_message(
            f"Лимит скорости загрузки обновлён: {format_download_speed_limit_label(bytes_per_second=bytes_per_second)}"
        )
    ...
    def _handle_update_tools_clicked(self) -> None:
        if self._tool_installation_controller.is_active or self._ytdlp_runtime_controller.is_active:
            self._show_transient_status_message("Проверка или обновление инструментов уже выполняется")
            return
    ...
    def _handle_clear_queue_clicked(self) -> None:
        should_confirm_clear_queue = self._settings.confirm_clear_queue and self._queue_table.has_tasks()
    ...
    def _cancel_tasks_download(self, task_ids: tuple[UUID, ...]) -> None:
        self._apply_download_update(update=self._download_controller.cancel_tasks_download(task_ids=task_ids))

    def _remove_tasks_from_queue(self, task_ids: tuple[UUID, ...]) -> None:
        self._apply_download_update(update=self._download_controller.remove_tasks_from_queue(task_ids=task_ids))
    ...
    def _maybe_offer_ytdlp_failure_recovery(
        ...
        *,
        ...
        failed_task_ids = tuple(task.task_id for task in update.updated_tasks if task.status is DownloadStatus.FAILED)
    ...
    def _apply_download_status_message(self, *, message: str) -> None:
        ...
        fallback_status_message = None if self._download_controller.is_active else DEFAULT_STATUS_MESSAGE
        self._show_transient_status_message(
            message,
    ...
    def _apply_speed_limit_indicator_visibility_directly(self, *, is_visible: bool) -> None:
        ...
        self._speed_limit_indicator_end_window_height = self._calculate_speed_limit_window_target_height(
        ...
        self._speed_limit_indicator_end_queue_height = self._calculate_speed_limit_queue_target_height(
            start_indicator_height=start_indicator_height,
            end_indicator_height=end_indicator_height,
        )
    ...
    def _handle_speed_limit_indicator_animation_value_changed(self, value: object) -> None:
        ...
        indicator_height = round(
            ...
            + (self._speed_limit_indicator_end_height - self._speed_limit_indicator_start_height) * progress
        )
        window_height = round(
            self._speed_limit_indicator_start_window_height
            + (self._speed_limit_indicator_end_window_height - self._speed_limit_indicator_start_window_height)
            * progress
        )
        queue_height = round(
            self._speed_limit_indicator_start_queue_height
            + (self._speed_limit_indicator_end_queue_height - self._speed_limit_indicator_start_queue_height) * progress
        )
    ...
    def _handle_speed_limit_indicator_animation_finished(self) -> None:
        ...
        is_expanding = self._speed_limit_indicator_end_height > self._speed_limit_indicator_start_height
    ...
    def _sync_queue_controls_state(self) -> None:
        ...
        has_downloadable_tasks = bool(self._container.download_queue_service.list_downloadable_tasks())
        ...
        has_environment_operation = (
            has_active_tool_installation or has_active_ytdlp_runtime_operation or has_active_browser_cookies_export
        )
