...
def test_download_started_status_is_primary() -> None:
    assert is_primary_download_status_message(message="Загрузка запущена: https://www.youtube.com/watch?v=test")


def test_download_canceling_status_is_primary() -> None:
    assert is_primary_download_status_message(message="Отмена загрузки... Частичные файлы будут удалены")
